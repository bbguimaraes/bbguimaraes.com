local function inline_code(x)
    return inline_tag(
        "pre", {class = "inline-code"},
        code_tag("code", nil, x:gsub("\n$", "")))
end

local description <const> = [[One of the principal differences between C and C++ is the latter's direct support for virtual functions.  While it is entirely possible, as will be demonstrated, to implement virtual dispatch in C, C++ offers language primitives that instruct the compiler to generate the necessary back end code automatically.]]
...
ABI</a>.  In particular:
    ...
    ul {
        tag("a",
            {href = "https://itanium-cxx-abi.github.io/cxx-abi/abi.html#vtable-components"},
            "section 2.5.2: <i>Virtual Table Components and Order</i>"),
        tag("a",
            {href = "https://itanium-cxx-abi.github.io/cxx-abi/abi.html#vtable-components"},
            "section 2.5.3: <i>Virtual Table Construction</i>"),
            tag("a",
                {href = "https://itanium-cxx-abi.github.io/cxx-abi/abi.html#vcall"},
...
document, with some ancillary  material around them:
        ...
        lines {
            ul {
                link { href = "#motivation", content = "Motivation" },
                link { href = "#code-generation", content = "Code generation" },
                lines {
                    html "Class categories",
                    ul {
                        link { href = "#trivial", content = "Trivial" },
                        link { href = "#leaf", content = "Leaf" },
                        tag("a",
                            {href="#general-single-inheritance"},
                            "General single inheritance"),
                        tag("a",
                            {href="#non-virtual-bases-only"},
                            "Non-virtual bases only"),
                    },
                },
                link { href = "#references", content = "References" },
...
presented at the end of this article:
    ...
    src_ref {
        href = "c++-virtual-tables/motivation/test.cpp",
        name = "test.cpp",
    },
    code [[
<b>struct</b> S { <b>virtual</b> <b>void</b> sf(<b>void</b>); };
...
Here is the C++ part of the program and its output:
    ...
    tag("details", nil, lines {
        inline_tag("summary", nil, "C++ source"),
        src_ref {
            href = "c++-virtual-tables/motivation/test.cpp",
            name = "test.cpp",
        },
...
things to your C++ compiler.
    ...
    h2_link { "code-generation", "Code generation" },
    par [[
...
explanations.
    ...
    tag("details", {class = "code-details"}, lines {
        inline_tag("summary", nil, "Examples"),
        code [[
$ cat test.c
<b>int</b> main(<b>void</b>) {
    <b>return</b> <b>0</b>;
...
and so none is generated and the objects do not contain a virtual pointer.
    ...
    div(nil, src_ref {
        href = "c++-virtual-tables/trivial.cpp",
        name = "trivial.cpp",
    }),
    inline_code [[
<b>struct</b> {
...
present at the beginning of each class instance.
    ...
    div(nil, src_ref {
        href = "c++-virtual-tables/leaf.cpp",
        name = "leaf.cpp",
    }),
    inline_code [[
<b>struct</b> S {
...
The equivalent code in pseudo-C would be:
        ...
        code [[
<b>extern</b> <b>u64</b> rdi;
<b>u64</b> **rax = *(<b>u64</b>***)rdi; <i>// mov</i>
<b>goto</b> **rax; <i>// jmp</i>
]],
        par [=[
...
since this is a leaf class).
    ...
    div(nil, src_ref {
        href = "c++-virtual-tables/leaf_fns_inline.cpp",
        name = "leaf_fns_inline.cpp",
    }),
    inline_code [[
<b>struct</b> S {
    <b>virtual</b> <b>void</b> f(<b>void</b>) {}
...
f:
    ...
    h2_link { "general-single-inheritance", "General single inheritance" },
    par [[
...
<code>details</code> elements below to view them.
    ...
    div(nil, src_ref {
        href = "c++-virtual-tables/single_inline.cpp",
        name = "single_inline.cpp",
    }),
    inline_code [[
<b>struct</b> S {
    <b>virtual</b> <b>void</b> sf(<b>void</b>) {}
...
<b>void</b> f(U *p) {
    ...
    tag("details", {class = "inline-code-details"}, lines {
        inline_tag("summary", nil, "Assembly"),
        code [=[
f(S*):
    <b>mov</b> <b>rax</b>, <b>QWORD</b> <b>PTR</b> [<b>rdi</b>]
    <b>jmp</b> [<b>QWORD</b> <b>PTR</b> [<b>rax</b>]]
...
s:
    ...
    tag("details", {class = "code-details"}, lines {
        inline_tag("summary", nil, "Implementation"),
        div(nil, src_ref {
            href = "c++-virtual-tables/single.c",
            name = "single.c",
        }),
        inline_code [=[
<b>struct</b> S;
...
S_vtable:
    ...
    h2_link { "non-virtual-bases-only", "Non-virtual bases only" },
    par [[
...
type and avoid some of the complications described below.
    ...
    html "<h3>Pointer adjustments</h3>",
    par [[
...
valid.
    ...
    div(nil, src_ref {
        href = "c++-virtual-tables/multiple_inline.cpp",
        name = "multiple_inline.cpp",
    }),
    inline_code [[
<b>struct</b> S {
    <b>virtual</b> <b>void</b> sf(<b>void</b>) {}
...
u:
    <b>.quad</b> vtable for U+16
    <b>.quad</b> vtable for U+56
]=],
    }),
    tag("details", {class = "code-details"}, lines {
        inline_tag("summary", nil, "Implementation"),
        div(nil, src_ref {
            href = "c++-virtual-tables/multiple_inline.c",
            name = "multiple_inline.c",
        }),
        inline_code [=[
<b>struct</b> S;
<b>struct</b> T;
...
u:
    <b>.quad</b> U_vtable+16
    <b>.quad</b> U_vtable+56
]=],
    }),
    h2_link { "references", "References" },
    par [[
...
which generates the same machine code is:
            ...
            src_ref {
                href = "c++-virtual-tables/leaf_inline.c",
                name = "leaf_inline.c",
            },
            code [[<b>void</b> f(<b>struct</b> S *p) {
...
We can emulate this in our C implementation, compare:
            ...
            div(nil, src_ref {
                href = "c++-virtual-tables/multiple_reload.cpp",
                name = "multiple_reload.cpp",
            }),
            inline_code [[
<b>void</b> f(U *p) {
    p-&gt;uf();
...
f(U*):
    ...
    <b>jmp</b> <b>rax</b>
            ...
            inline_tag("p", nil, "vs."),
            div(nil, src_ref {
                href = "c++-virtual-tables/multiple_reload.c",
                name = "multiple_reload.c",
            }),
            inline_code [[
<b>void</b> f_U(<b>struct</b> U *p) {
...
this relation exists for a pair of types:
            ...
            ul {
                link {
                    href = "https://en.cppreference.com/w/cpp/types/is_layout_compatible",
                    content = "std::is_layout_compatible",
                },
                link {
                    href = "https://en.cppreference.com/w/cpp/types/is_pointer_interconvertible_base_of",
                    content = "std::is_pointer_interconvertible_base_of",
                },
                link {
                    href = "https://en.cppreference.com/w/cpp/types/is_pointer_interconvertible_with_class",
                    content = "std::is_pointer_interconvertible_with_class",
                },
                link {
                    href = "https://en.cppreference.com/w/cpp/types/is_corresponding_member",
                    content = "std::is_corresponding_member",
                },
            },
        }),
        tag("li", {id = "note6"}, lines {
            par [[
